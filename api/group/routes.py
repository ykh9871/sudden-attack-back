...
from app.group.dto.dto import Group, GroupCreate, GroupMembershipRequest, MemberRequestsView, AdminUser
...
async def create_group(group_create: GroupCreate, user_id: UserID = Depends(user_service.get_userid_by_email)):
...
async def delete_group(group_id: int, user_id: UserID = Depends(user_service.get_userid_by_email)):
    try:
...
async def study_group_join_request(group_id: int, user_id: UserID = Depends(user_service.get_userid_by_email)):
...
async def get_member_requests_by_user(user_id: UserID = Depends(user_service.get_userid_by_email)):
...
async def get_member_requests(group_id: int, user_id: UserID = Depends(user_service.get_userid_by_email)):
...
async def get_all_members(group_id: int, current_user_id: UserID = Depends(user_service.get_userid_by_email)):
    return group_service.get_all_members(group_id, current_user_id)

@router.delete("/group/group_withdrawal")
async def group_withdrawal(group_id: int, current_user_id: UserID = Depends(user_service.get_userid_by_email)):
