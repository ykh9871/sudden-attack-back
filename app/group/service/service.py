...
class GroupService:
    ...
    def get_member_requests(self, group_id: int, user_id: int) -> List[MemberRequestsView]:
        role = self.group_repository.get_member_role(group_id, user_id)
        if role == "ADMIN":
            return self.group_repository.get_member_requests(group_id)
        else:
            raise HTTPException(status_code=403, detail="Only group administrators can view member requests.")
    ...
    def group_withdrawal(self, group_id: int, current_user_id: int):
        role = self.group_repository.get_member_role(group_id, current_user_id)
        if role == "ADMIN":
            return HTTPException(status_code=403, detail="Group administrators are not allowed to leave.")
