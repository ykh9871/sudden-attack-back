from app.group.dto.dto import Group, GroupCreate, GroupMembershipRequest, MemberRequestsView, AllMembers, AdminUser
...
class GroupRepository:
    ...
    # 새로운 그룹을 데이터베이스에 추가합니다.
        ...
        if group_id is not None:
            ...
            # 새로운 그룹을 반환합니다.
            return Group(id=group_id, name=group_create.name, description=group_create.description, created_at=created_at)
        else:
    ...
    def get_member_requests_by_user(self, user_id: UserID) -> List[GroupMembershipRequest]:
        self.cursor.execute(
        ...
        requests = []
        for row in rows:
            requests.append(GroupMembershipRequest(
                group_name = row[0],
                created_at = row[1]
            ))
        return requests
    ...
    def get_member_requests(self, group_id: int) -> List[MemberRequestsView]:
        ...
        requests = []
        for row in rows:
            requests.append(MemberRequestsView(
                id = row[0],
                username=row[1],
                nickname=row[2],
    ...
    def get_all_members(self, group_id: int, role: str, current_user_id: int):
        ...
        for member in members:
            members_info.append(AllMembers(
                id = member[0],
                nickname=member[1],
            ))
    ...
    def get_all_groups(self, name: Optional[str] = None):
        if name:
            self.cursor.execute(
                ...
                "FROM 'group' g "
                "LEFT JOIN group_member gm ON gm.group_id = g.id AND gm.role != 'PENDING' "
                "WHERE g.name LIKE ? "
                "GROUP BY g.id",
                ('%' + name + '%',)
            )
